...
const createContact = async (req, res) => {
    ...
    if (result.acknowledged) {
        ...
        res.status(500).json(result.error || 'Error occured creating contact.');
    }
};
...
const updateContact = async (req, res) => {
    ...
    if (result.modifiedCount > 0) {
        ...
        res.status(500).json(result.error || 'Error occured updating contact.');
    }
};
...
const deleteContact = async (req, res) => {
    ...
    if (result.deletedCount > 0) {
        ...
        res.status(500).json(result.error || 'Error occured deleting contact.');
    }
};
